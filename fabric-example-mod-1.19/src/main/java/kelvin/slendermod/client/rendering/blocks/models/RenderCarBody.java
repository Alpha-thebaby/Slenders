...
public class RenderCarBody extends GeoBlockRenderer<CarBodyBlockEntity> {
    ...
    @Override
    public void actuallyRender(MatrixStack poseStack, CarBodyBlockEntity animatable, BakedGeoModel model, RenderLayer renderType, VertexConsumerProvider bufferSource, VertexConsumer buffer, boolean isReRender, float partialTick, int packedLight, int packedOverlay, float red, float green, float blue, float alpha) {
        poseStack.push();
